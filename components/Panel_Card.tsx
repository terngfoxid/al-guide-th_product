...
export default function Panel_Card() {
    ...
    type dataType = {
        data: {
            data_update: [{ time: { _seconds: number }, update_note: [string] }] | { error: any | string, length: number },
            web_update: [{ time: { _seconds: number }, update_note: [string] }] | { error: any | string, length: number }
        }
    }
    ...
    const card_style = (
        {
            ...
            //list
            list_style: "inline-block py-1 mt-1 px-4 md:border-b-2 border-transparent rounded-lg md:rounded-t-lg md:rounded-b-none hover:text-gray-600 hover:border-gray-600 dark:hover:text-gray-300",
                        ...
                        <div className={card_style.body_style}>
                            <div className="flex justify-center">
                                <ul className="flex flex-wrap -mb-px w-11/12">
                                    <li className="mr-2">
                                        <button id="data_update" className={mode == 0 ? card_style.list_select_style : card_style.list_style}>อัพเดตข้อมูล</button>
                                    </li>
                                    <li className="mr-2">
                                        <button id="web_update" className={mode == 1 ? card_style.list_select_style : card_style.list_style}>อัพเดตเว็บ</button>
                                    </li>
                                    <li className="mr-2">
                                        <button id="basic_qa" className={mode == 2 ? card_style.list_select_style : card_style.list_style}>ปัญหาการใช้งานเว็บไซต์เบื้องต้น</button>
                                    </li>
                                </ul>
                            </div>
                            <div className="block md:hidden"><div className={card_style.cut_style}></div></div>
                            <div className="flex justify-center md:mt-0.5">
                                <div className={card_style.content_style + " " + card_style.mode_style[mode] + " flex justify-center mb-2 py-2"}>
    ...
    if (!Array.isArray(data.data.data_update)) {
        ...
        for (count = 0; count < data.data.data_update.length; count++) {
            ...
            if (data.data.data_update[buffer] != null) {
                ...
                let line = 0
                for (line = 0; line < data.data.data_update[buffer].update_note.length; line++) {
                ...
                if (buffer != data.data.data_update.length - 1) {
                    data_d.push(<div className="flex justify-center"><div className={card_style.end_content_style}></div></div>)
                }
            }
        }
    }
    ...
    if (!Array.isArray(data.data.web_update)) {
        ...
        for (count = 0; count < data.data.web_update.length; count++) {
            ...
            if (data.data.web_update[buffer] != null) {
                ...
                let line = 0
                for (line = 0; line < data.data.web_update[buffer].update_note.length; line++) {
                    const line_buffer = line
                    data_w.push(
                        <p className={card_style.sub_content_body_style}>
                ...
                if (buffer != data.data.web_update.length - 1) {
                    data_w.push(<div className="flex justify-center"><div className={card_style.end_content_style}></div></div>)
                }
            }
        }
    }
                ...
                <div className={card_style.shape}>
                    ...
                    <div className={card_style.body_style}>
                        <div className="flex justify-center">
                            <ul className="flex flex-wrap -mb-px w-11/12">
                                <li className="mr-2">
                                    <button id="data_update" className={mode == 0 ? card_style.list_select_style : card_style.list_style} onClick={click_data}>อัพเดตข้อมูล</button>
                                </li>
                                <li className="mr-2">
                                    <button id="web_update" className={mode == 1 ? card_style.list_select_style : card_style.list_style} onClick={click_web}>อัพเดตเว็บ</button>
                                </li>
                                <li className="mr-2">
                                    <button id="basic_qa" className={mode == 2 ? card_style.list_select_style : card_style.list_style} onClick={click_qa}>ปัญหาการใช้งานเว็บไซต์เบื้องต้น</button>
                                </li>
                            </ul>
                        </div>
                        <div className="block md:hidden"><div className={card_style.cut_style}></div></div>
                        <div className="flex justify-center md:mt-0.5">
                            {mode == 0 ? <><div className={card_style.content_style + " " + card_style.mode_style[mode]}>{data_d}</div>
                            </> :
                                mode == 1 ? <><div className={card_style.content_style + " " + card_style.mode_style[mode]}>{data_w}</div>
                                </> :
                                    mode == 2 ? <><div className={card_style.content_style + " " + card_style.mode_style[mode]}>
                                        <p className={card_style.sub_content_head_style}>1.ข้อมูลของตัวละครไม่มีการอัปเดต</p>
                                        <p className={card_style.sub_content_body_style}>เนื่องจากเรามีการเก็บข้อมูลบางส่วนไว้ใน Browser แบบ Offline</p>
                                        <p className={card_style.sub_content_body_style}>ทำให้ข้อมูลบางอย่างที่เก็บไว้แบบ offline ไม่มีการอัปเดต</p>
                                        ...
                                        <p className={card_style.sub_content_body_style}>***ขออภัยในความไม่สะดวก***</p>
                                    </div>
                                    </> : <></>}
                        </div>
                    </div>
