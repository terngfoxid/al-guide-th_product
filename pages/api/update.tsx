...
export default async function handler(req: any, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: FirebaseFirestore.DocumentData | undefined): void; new(): any; }; }; }) {
  ...
  const snapshot_d = await db.collection('data_update').orderBy('id', 'desc').limit(5).get();
  const snapshot_w = await db.collection('web_update').orderBy('id', 'desc').limit(5).get();
  ...
  if (snapshot_d.empty) {
    data.data_update = {error_d: "Not Found Data Update" };
  }
  else {
  ...
  if (snapshot_w.empty) {
    data.data_update = {error_w: "Not Found Web Update" };
  ...
  if (data == null) { res.status(404).json({ error: "Not Found Update Data" }) }
